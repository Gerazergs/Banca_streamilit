...
if str(numero).lower() == st.session_state.name.lower():
    with st.spinner("Por favor espere..."):
        ...
        df_5['CLIENTES ACTIVOS'] = df_5['CLIENTES ACTIVOS'].str.replace(',','').astype('float').astype('int64')
        df_5['PPTO 2022'] = df_5['PPTO 2022'].str.replace(',','').astype('float').astype('int64')
       ...
       'Monetarias', 'No Monetarias', 'Total']].astype(int)
        ...
        tr_col1.metric("Monetarias", f"{round(df_4['Monetarias'].sum()/1000000,2)}M", f"{round((df_4['Monetarias'].sum()/df_4['TX  Monetarias presupuesto'].sum())-1,2)}%")
        tr_col2.metric("No Monetarias",  f"{round(df_4['No Monetarias'].sum()/1000000,2)}M", f"{round((df_4['No Monetarias'].sum()/df_4['TX No Monetarias presupuesto'].sum())-1,2)}%")
        tr_col3.metric("Total",  f"{round(df_4['Total'].sum()/1000000,2)}M", f"{round((df_4['Total'].sum()/df_4['TX Totales Presupuesto'].sum())-1,2)}%")
