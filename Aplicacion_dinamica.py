...
if str(numero).lower() == st.session_state.name.lower():
    with st.spinner("Por favor espere..."):
       ...
       'Monetarias', 'No Monetarias', 'Total']].astype(int)
        ...
        tr_col1.metric("Monetarias", df_4['Monetarias'].sum()/1000000, 1)#{:.2f}".format((df_4['Monetarias'].sum()/df_4['TX  Monetarias presupuesto'].sum())-1))
        tr_col2.metric("No Monetarias",  df_4['No Monetarias'].sum()/1000000, 1)#"{:.2f}".format((df_4['No Monetarias'].sum()/df_4['TX No Monetarias presupuesto'].sum())-1))
        tr_col3.metric("Total",  df_4['Total'].sum()/1000000, 1)#"{:.2f}".format((df_4['Total'].sum()/df_4['TX Totales Presupuesto'].sum())-1))
